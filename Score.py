...
class Score():
  def __init__(self):
      self.score_file_name = Const.SCORES_FILE_NAME
  ...
  def read_score(self):
      if os.path.isfile(self.score_file_name):
        try:
          ...
          score_value = dictionary.get(Const.SCORES_LABEL, 0)
          score_file.close()
          return score_value
        except:
          return Const.BAD_RETURN_CODE
      else:
        return 0
